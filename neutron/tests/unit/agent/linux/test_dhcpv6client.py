...
from neutron.agent.linux import dhcpv6client
from neutron.agent.linux.dhcpv6client import client
from neutron.agent.linux.dhcpv6client import config  # noqa
from neutron.agent.linux.dhcpv6client import constants
from neutron.agent.linux.dhcpv6client import dhcpv6
from neutron.agent.linux.dhcpv6client import exceptions
from neutron.agent.linux.dhcpv6client import listener
from neutron.agent.linux.dhcpv6client import socketv6
from neutron.agent.linux.dhcpv6client import utils
from neutron.agent.linux import pd_driver_config as pd_config
...
class TestDHCPV6(base.BaseTestCase):
    ...
    def test_gen_trid(self):
        ...
        self.assertTrue(not id1 == id2)

    def test_bytes_to_int(self):
        results = dhcpv6.bytes_to_int(b'\x19')
        self.assertTrue(results == 25)
...
class TestDHCPv6API(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.pd_manager = dhcpv6client.PDManager("router", "subnet", "blah")
    ...
    def test_get_prefix_error_handling(self):
        ...
        self.assertRaises(
            exceptions.DHCPv6ClientException, self.pd_manager.get_prefix)
        self.socket().send.assert_called_once_with('get,subnet,uuid,')

    def test_sync_data(self):
        response = dhcpv6client.get_sync_data()
...
class TestClient(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.subnetpd = mock.patch(
            'neutron.agent.linux.dhcpv6client.subnetpd.SubnetPD').start()
        self.listener = mock.patch(
            'neutron.agent.linux.dhcpv6client.listener.start').start()
        ...
        self.conf.register_opts(pd_config.OPTS)

    def test_notify_l3_agent(self):
        with mock.patch('os.kill') as kill:
            client.notify_l3_agent(12345)
    ...
    @mock.patch('os.listdir')
    @mock.patch('__builtin__.open')
    def test_DHCPv6Client_system_load_listdir_exception(
    ...
    @mock.patch('os.listdir')
    @mock.patch('__builtin__.open')
    def test_DHCPv6Client_system_load_open_exception(
            self, mock_open, mock_listdir, mock_chmod, mock_remove):
        subnet_ids = ["subnet_1", "subnet_2", "subnet_3", "subnet_4"]
        mock_listdir.return_value = subnet_ids
    ...
    @mock.patch('os.listdir')
    @mock.patch('__builtin__.open')
    def test_DHCPv6Client_processor_enable(
            self, mock_open, mock_listdir, mock_chmod, mock_remove):
        dc = client.DHCPV6Client()
    ...
    def test_DHCPv6Client_processor_disable(
    ...
    @mock.patch('os.listdir')
    @mock.patch('__builtin__.open')
    def test_DHCPv6Client_processor_get(
            self, mock_open, mock_listdir, mock_chmod, mock_remove):
        dc = client.DHCPV6Client()
        subnetpd = mock.Mock()
...
class TestSocketV6(base.BaseTestCase):
    ...
    @mock.patch('neutron.agent.linux.dhcpv6client.socketv6.socket_v6')
...
class TestListener(base.BaseTestCase):
    ...
    @mock.patch('neutron.agent.linux.dhcpv6client.socketv6.socket_v6')
    @mock.patch('neutron.agent.linux.dhcpv6client.utils.new_daemon_thread')
    @mock.patch('neutron.agent.linux.dhcpv6client.listener.SOCKET')
